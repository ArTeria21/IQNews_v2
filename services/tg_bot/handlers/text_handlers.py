...
async def dont_understand_text(message: types.Message):
    ...
    logger.warning(f"Бот получил некорректное сообщение <{message.text}> от пользователя {message.from_user.id}", correlation_id=correlation_id)
...
async def edit_preferences(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Бот получил обновление предпочтений пользователя {message.from_user.id}", correlation_id=correlation_id)
    ...
    logger.info(f'Изменения предпочтений пользователя {message.from_user.id} сохранены', correlation_id=correlation_id)
...
async def edit_keywords(message: types.Message, state: FSMContext):
    ...
    connection = await get_rabbit_connection()
    logger.info(f"Бот получил обновление ключевых слов пользователя {message.from_user.id}", correlation_id=correlation_id)
    async with connection.channel() as channel:
    ...
    logger.info(f'Изменения ключевых слов пользователя {message.from_user.id} сохранены', correlation_id=correlation_id)
...
async def subscribe_feed(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Бот получил запрос на подписку на {feed_url} от пользователя {message.from_user.id}", correlation_id=correlation_id)
    await state.clear()
    if not await is_valid_rss_feed(feed_url):
        await message.reply(INVALID_FEED_URL_TEXT)
        logger.warning(f"Некорректный URL RSS-канала {feed_url} от пользователя {message.from_user.id}", correlation_id=correlation_id)
        return
    if not await is_feed_active(feed_url):
        await message.reply(INACTIVE_FEED_TEXT)
        logger.warning(f"RSS-канал {feed_url} не активен", correlation_id=correlation_id)
        return
    ...
    logger.info(f"Подписка на RSS-канал {feed_url} от пользователя {message.from_user.id} сохранена", correlation_id=correlation_id)
...
async def unsubscribe_feed(message: types.Message, state: FSMContext):
    ...
    logger.info(f"Бот получил запрос на отписку от RSS-канала {feed_url} от пользователя {message.from_user.id}", correlation_id=correlation_id)
    connection = await get_rabbit_connection()
    async with connection.channel() as channel:
    ...
    logger.info(f"Отписка от RSS-канала {feed_url} от пользователя {message.from_user.id} сохранена", correlation_id=correlation_id)